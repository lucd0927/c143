...
class ParticlePainter extends CustomPainter {
  ...
  ParticleData? hitTest2(Offset tapPosition) {
    final currentTime = DateTime.now();
    final elapsedTime =
    ...
    // 从上层粒子开始检测
    for (final particle in particles.reversed) {
      final position =
      _calculatePosition(screenSize, particle, timeProgress);

      final radius = particle.size / 2;

      // 用圆形命中（最快 & 最稳）
      if ((tapPosition - position).distance <= radius) {
        return particle;
      }
    }
    return null;

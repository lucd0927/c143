...
import com.dexterous.flutterlocalnotifications.broadcast.BroadcastNotificationReceiver;
import com.dexterous.flutterlocalnotifications.broadcast.FlutterBroadcastNotificationPlugin;
...
public class FlutterLocalNotificationsPlugin
  ...
  private static final String GET_MESSAGE_RECEIVED_NUM = "getMessageReceivedNum";
  private static final String SET_MESSAGE_RECEIVED_NUM = "setMessageReceivedNum";
  private static final String SHOW_BROADCAST_NOTIFICATION = "showBroadcastNotification";
  ...
  private static final String SUBSCRIBE_TO_TOPIC_METHOD = "subscribeToTopic";
  ...
  public static void createNotification(
      Context context, NotificationDetails notificationDetails, NotificationBuildListener listener) {
  ...
  private static void setupNotificationChannel(
    ...
    if (VERSION.SDK_INT >= VERSION_CODES.O) {
      ...
      if (BooleanUtils.getValue(notificationChannelDetails.playSound)) {
...
//        int id = generateNotifyId();
  ...
  public void onDetachedFromEngine(@NonNull FlutterPluginBinding binding) {
    ...
    this.channel = null;
    BroadcastNotificationReceiver.removeAll(applicationContext);
  ...
  public void onMethodCall(MethodCall call, @NonNull Result result) {
    switch (call.method) {
      ...
      case STOP_FOREGROUND_SERVICE:
        ...
        break;
      case SUBSCRIBE_TO_TOPIC_METHOD:
        subscribeToTopic(call, result);
        break;
      case GET_MESSAGE_RECEIVED_NUM:
        getMessageReceivedNum(call, result);
        break;
      case SET_MESSAGE_RECEIVED_NUM:
        setMessageReceivedNum(call, result);
        break;
      case SHOW_BROADCAST_NOTIFICATION:
        showBroadcastNotification(call, result);
        break;
      default:
  ...
  private void showBroadcastNotification(MethodCall call, Result result) {
    final String broadcast = call.argument("broadcast");
    final int interval = call.argument("interval");
    FlutterBroadcastNotificationPlugin.saveInterval(applicationContext, interval, broadcast);
    NotificationDetails notificationDetails = extractNotificationDetails(result, call.arguments());
    FlutterBroadcastNotificationPlugin.saveNotificationDetails(applicationContext, notificationDetails, broadcast);
    BroadcastNotificationReceiver.add(applicationContext, broadcast);
    result.success(true);
  }

  private void getMessageReceivedNum(MethodCall call, Result result) {
    final String payload = call.argument("payload");
    result.success(FlutterFCMPlugin.getMessageReceivedNum(applicationContext, payload));
  }

  private void setMessageReceivedNum(MethodCall call, Result result) {
    final String payload = call.argument("payload");
    final int num = call.argument("num");
    FlutterFCMPlugin.setMessageReceivedNum(applicationContext, payload, num);
    result.success(true);
  }

  private void subscribeToTopic(MethodCall call, Result result) {
    final String topic = call.argument("topic");
    NotificationDetails notificationDetails = extractNotificationDetails(result, call.arguments());
    FlutterFCMPlugin.saveNotificationDetails(applicationContext, notificationDetails);
